...
int main()
{
	...
	while (window.isOpen())
	{
		...
		if (Keyboard::isKeyPressed(Keyboard::E)) // ¬з€ть предмет
		{
		//	view.zoom(0.995);
		...
		if (Keyboard::isKeyPressed(Keyboard::T)) // ¬ыложить предмет
		{
			//	view.zoom(0.995);
		...
		if (Keyboard::isKeyPressed(Keyboard::Add)) // ѕриближение камеры
		{
			view.zoom(0.995);
		}
		if (Keyboard::isKeyPressed(Keyboard::Subtract))// ќтдаление камеры
		{
			view.zoom(1.005);
		}
